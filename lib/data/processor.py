...
def prepare_data(dataset_name, time_offset_q=None):
    time_offset_test, time_offset_valid = read_time_offsets(time_offset_q)

    data = read_raw_data(dataset_name)
    userid, itemid, timeid = entity_names(dataset_name)
    train_data_, test_data_ = split_data_by_time(
        data, time_offset_test, timeid
    )
    test_datapack = reindex_data(
        train_data_.copy(), test_data_, userid, itemid
    )
    tune_datapack = None
    if time_offset_valid is not None: # validation split
        train_data_valid_, valid_data_ = split_data_by_time(
            train_data_, time_offset_valid, timeid
        )
        tune_datapack = reindex_data(
            train_data_valid_.copy(), valid_data_, userid, itemid
        )
    return tuple(filter(None, [tune_datapack, test_datapack]))

def read_time_offsets(time_offset_q):
    if time_offset_q is None:
        time_offset_q = defaults.time_offset_q
    time_offset_test = time_offset_q
    time_offset_valid = None
    if isinstance(time_offset_q, (list, tuple)): # validation-test
        time_offset_valid, time_offset_test = time_offset_q
    return time_offset_test,time_offset_valid
...
def split_data_by_time(data, time_q, timeid):
